...
export const findUserById = catchAsyncErrors(async (req, res, next) => {
  const patientId = req.params.patientId;
  if (!patientId) {
    return next(new ErrorHandler("Patient ID is required.", 400));
  }

  const patients = await User.findById(patientId);
  if (!patients) {
    return next(new ErrorHandler("patients not found for this patient.", 404));
  }
...
// export const getAllPatients = catchAsyncErrors(async (req, res, next) => {
//   const patients = await User.find({ role: "doctor" });
